# CSW Protocol Compatibility Matrix
**Document:** `CSW_PROTOCOL_COMPATIBILITY_MATRIX.md`  
...
**Status:** Engineering Deliverable — Reviewed & Corrected Compatibility Matrix  

---

## 1. Executive Summary

This document presents a rigorous architectural, protocol-level, and empirical compatibility matrix between the implemented **CSWMobile** Android Native client and the upstream **ChatServer** (C# / .NET).

* **Android Implementation Status:** The Android implementation contains eight primary Kotlin source files, in addition to Gradle configuration, manifest, and resource files. It incorporates Jetpack Compose Material 3 user interfaces, an asynchronous TCP socket engine based on Kotlin Coroutines (`Dispatchers.IO`), protocol serialization routines, and state-managed ViewModels.
* **Build Verification:** The Android project compiled successfully using the Gradle configuration currently present in the repository (`Build succeeded - the applet is compiled`).
* **Network & Integration Verification:** Not performed. No live network integration tests or end-to-end socket handshakes have been conducted against an active `ChatServer` instance.
* **Server-Side Alignment:** The current Android implementation was constructed using inferred protocol structures derived from prior architectural notes. Direct source access to the upstream C# codebase (`ChatProtocol.cs`, `ChatServer.cs`, `appsettings.json`) was not available during development.

### Official Verdict
> **Build Successful — Protocol Compatibility Not Verified**  
> *(Status: Partially Implemented Android Client — Compatibility Not Verified)*
...
2. **C# Source Unavailability:** The original upstream C# repository files (`ChatServer.cs`, `ChatProtocol.cs`, `ServerForm.cs`, `CallHub.cs`, `CollaborationHub.cs`) were not directly uploaded or parsed.
...
The Android implementation contains eight primary Kotlin source files, in addition to Gradle configuration, manifest, and resource files, as documented below:
...
| `/app/build.gradle.kts` | Build Configuration | Gradle Build Script | Gradle / AGP | Version Catalog, Coil, Room, Compose | Validated & Building | `[Confirmed in Android]` |
...
| :--- | :--- | :--- | :--- | :--- |
| **Total Header Size** | 8 bytes (`HEADER_SIZE = 8`) | Unknown until C# header parser is examined | `[Engineering Assumption]` | Verify `ChatProtocol.cs` field declarations. |
| **MessageType Field** | 4 bytes, 32-bit signed integer | Unknown (could be `int`, `short`, `byte`, or enum) | `[Requires C# Source]` | Check underlying enum type: `enum MessageType : int` vs `: byte`. |
...
| **Endianness** | Little-Endian (`ByteOrder.LITTLE_ENDIAN`) | Little-Endian is assumed because standard .NET primitive binary writing is commonly little-endian, but the actual server implementation must be inspected before confirming this behavior | `[Engineering Assumption]` | Confirm whether custom byte swapping (`BitConverter` / `BinaryPrimitives`) is present in C#. |
| **Fragmentation Assembly**| `readFully()` loops until all `length` bytes arrive | Socket reader loop | `[Confirmed in Android]` | Verify that packet assembly is independent of TCP segment boundaries and correctly handles fragmented and coalesced reads. |
...
| **EOF / Disconnect Detection**| Detects EOF when the input stream returns -1; zero-byte behavior depends on the underlying stream implementation | Catches zero-byte reads or socket exceptions | `[Confirmed in Android]` | Verify clean teardown and exception handling on both sides. |
...
| `AUTH_REQUEST` | 1 | `Auth` / `AuthRequest` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `AUTH_SUCCESS` | 2 | `AuthSuccess` / `LoginSuccess` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `AUTH_FAILED` | 3 | `AuthFailed` / `LoginFailed` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `CHAT_MESSAGE` | 4 | `ChatMessage` / `Message` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `SEND_FILE` | 5 | `SendFile` / `FileTransfer` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `FILE_DATA_CHUNK` | 6 | `FileData` / `FileChunk` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `FILE_ACK` | 7 | `FileAck` / `FileReceived` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `COLLABORATION_SNAPSHOT` | 8 | `CollaborationSnapshot` / `Snapshot` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `COMMAND` | 9 | `Command` / `CollaborationCommand` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `PRESENCE_UPDATE` | 10 | `PresenceUpdate` / `UserStatus` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `TYPING_INDICATOR` | 11 | `Typing` / `TypingIndicator` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `DISCONNECT` | 12 | `Disconnect` / `Leave` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |
| `HEARTBEAT` | 13 | `Heartbeat` / `Ping` | Unknown | `[Requires C# Source]` | Not verifiable without original C# enum source. |

> **Critical Note:** None of the integer opcode assignments (1 through 13) can be certified as compatible without direct inspection of the `MessageType` enum defined within `ChatProtocol.cs` (or the specific file that defines the enum in the C# solution).
...
| :--- | :--- | :--- | :--- |
| **Local Video Recording** | Not implemented | Reported as present in the desktop project, but not independently verified in this audit | Purely local camera recording to an `.mp4` file. Not a network stream. |
...
| :--- | :--- | :--- | :--- | :--- |
| **TCP Socket Engine** | Fully implemented (`TcpSocketManager`) | `TcpListener` confirmed in architecture | No | `[Theoretically Compatible — Requires Integration Test]` |
...
| **Text Chat Messaging** | Implemented with M3 UI bubbles | Inferred from reports | No | `[Theoretically Compatible — Requires C# Source]` |
...
1. **[BLOCKER 1] Absence of Upstream C# Source Code:** Inability to inspect `ChatProtocol.cs` and `ChatServer.cs` prevents definitive validation of opcodes, header length, and endianness.
...
* [ ] Align integer constants in `MessageType.kt` with the official enum in `ChatProtocol.cs`.
...
Based strictly on the source code present in the repository, the successful compilation status, and the absence of live empirical network tests:
...
================================================================================
  ...
  The Android client is implemented and compiles successfully. It contains
  a TCP socket layer, packet serialization logic, authentication flow,
  Compose-based screens, and preliminary collaboration models.

  However, the actual compatibility with ChatServer cannot be certified
  because the authoritative C# protocol implementation was not available
  for direct inspection and no live integration test was executed.

  The header structure, opcode values, authentication sequence, JSON
  schemas, file-transfer protocol, and SignalR routes remain unverified.

  Therefore, the current implementation must be classified as a
  partially implemented Android client whose cross-platform compatibility
  is pending protocol verification and empirical LAN testing.
================================================================================
```

---
*Report generated and committed to project root.*  
*File Name:* `CSW_PROTOCOL_COMPATIBILITY_MATRIX.md`
